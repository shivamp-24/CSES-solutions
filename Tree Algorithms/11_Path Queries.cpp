...
int query_path(int a, int b, int n){
    ...
    for(; head[a] != head[b]; b = parent[head[b]]) {
        ...
        int cur_heavy_path = query(1, 0, n-1, pos[head[b]], pos[b]); //query in segment tree to find for current heavy path
        res += cur_heavy_path;
    }
    //now both a and b part of same heavy path
    if(depth[a] > depth[b]) swap(a, b);
    int last_heavy_path = query(1, 0, n-1, pos[a], pos[b]);
    res += last_heavy_path;
    return res;
}
